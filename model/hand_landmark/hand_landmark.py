    ...
    def __postprocess(
        ...
        for resized_image, resize_scale_224x224, half_pad_size_224x224, rect, xyz_x21, left_hand_0_or_right_hand_1 in \
            ...
            for x,y in rescaled_xy:
                ...
                x_ls = new_coord[0]
                y_ls = new_coord[1]
                x_val = float(x_ls) if not hasattr(x_ls, "item") else float(x_ls.item())
                y_val = float(y_ls) if not hasattr(y_ls, "item") else float(y_ls.item())
                keypoints.append([int(x_val), int(y_val)])
            ...
            hand_flag = left_hand_0_or_right_hand_1
            if hasattr(hand_flag, "item"):
                hand_flag = hand_flag.item()

            rotated_image_size_leftrights.append(
                [rotated_image_width, rotated_image_height, float(hand_flag)]
            )
